...
import com.example.week11_roodb.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    val adapter = MyAdapter(ArrayList<Product>())
    ...
    private fun initRecyclerView() {
        ...
        adapter.itemClickListener = object :MyAdapter.OnItemClickListener{
            override fun OnItemClick(position: Int) {
                binding.pIdEdit.setText(adapter.items[position].pId)
                binding.pNameEdit.setText(adapter.items[position].pName)
                binding.pQuantityEdit.setText(adapter.items[position].pQuantity)
            }
        }
